...
📌 *Note*: Some commands are owner-only`;
        ...
        await malvin.sendMessage(
            from,
            {
                image: { url: `https://i.ibb.co/x8hP1DR4/shaban-md.jpg` }, // Replace with privacy-themed image if available
                caption: privacyMenu,
                contextInfo: {
...
async (malvin, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        reply(`📋 \`Gimaa-MD BLOCKED USERS (${count})\`:\n\n${list}`);
    } catch (err) {
        console.error(err);
