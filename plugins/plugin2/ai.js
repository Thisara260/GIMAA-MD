...
malvin({
    ...
    category: "ai",
    react: "🤖",
    use: ".gimaa <your query>",
    filename: __filename,
}, async (malvin, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for Gimaa Ai.\n\nExample:\n.Gimaa What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Gimaa API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://i.ibb.co/x8hP1DR4/shaban-md.jpg'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Gimaa's Response:*\n\n${gptResponse}`;
...
malvin({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://i.ibb.co/x8hP1DR4/shaban-md.jpg'; // Replace with a valid image URL
